...
import { PlatformUtils, ObjectUtils, ArrayUtils, StringUtils, MathUtils, UrlUtils } from "../utilities"
import { ScreenVolume, ColorTable } from "../viewer"
import { Menu, MenuItem, Dialog, MenuItemCheckBox, MenuItemFileChooser, MenuItemRadioButton, MenuItemRange, MenuItemSlider, MenuItemSpacer } from "./"
...
export class Toolbar {
  ...
  static ICON_IMAGESPACE = "data:image/gif;base64,R0lGODlhFAAUAPcAMf//////GP////////////////////////////////" +
    "////////////////////////////////////////////////////////////////////////////////////////////////////////////////" +
  ...
  static ICON_WORLDSPACE = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABUAAAAVCAYAAACpF6WWAAAAGXRFWHRTb2" +
  ...
  static ICON_EXPAND = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAIAAAACACAYAAADDPmHLAAAE00lEQVR42u2d" +
  ...
  static ICON_COLLAPSE = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABYAAAAWCAYAAADEtGw7AAAEJGlDQ1BJQ0Mg" +
  ...
  static FILE_MENU_DATA = {
    "label": "File", "icons": null,
    "items": [
      { "label": "Add Image...", "action": "OpenImage", "type": "file", "hide": PlatformUtils.ios },
      { "label": "Add Surface...", "action": "OpenSurface", "type": "file", "hide": PlatformUtils.ios },
      {
        "label": "Add DICOM Folder...", "action": "OpenFolder", "type": "folder",
        "hide": ((PlatformUtils.browser !== "Chrome") || ((typeof (daikon) === "undefined")))
      },
  ...
  static MENU_DATA = {
    "menus": [
      ...
      {
        "label": "EXPAND", "icons": [Toolbar.ICON_EXPAND, Toolbar.ICON_COLLAPSE], "items": [],
        "method": "isCollapsable", "required": "isExpandable"
      },
      {
        "label": "SPACE", "icons": [Toolbar.ICON_IMAGESPACE, Toolbar.ICON_WORLDSPACE],
        "items": [], "method": "isWorldMode", "menuOnHover": true
      }
    ]
  };

  static MENU_DATA_KIOSK = {
    "menus": [
      {
        "label": "EXPAND", "icons": [Toolbar.ICON_EXPAND, Toolbar.ICON_COLLAPSE], "items": [],
        "method": "isCollapsable", "required": "isExpandable"
  ...
  static OVERLAY_IMAGE_MENU_DATA = {
    "items": [
  ...
  static BASE_IMAGE_MENU_DATA = {
    "items": [
      { "label": "Show Header", "action": "ShowHeader" },
      { "label": "Show Image Info", "action": "ImageInfo" },
      { "label": "DisplayRange", "action": "ChangeRange", "type": "displayrange", "method": "getRange" },
      Toolbar.OVERLAY_IMAGE_MENU_DATA.items[6],
      {
  ...
  static DTI_IMAGE_MENU_DATA = {
    "items": [
      ...
      { "label": "Modulate with...", "action": "DTI-Mod", "type": "file", "hide": PlatformUtils.ios, "required": "canCurrentOverlayLoadMod" },
      { "label": "Modulation", "action": "dtiAlphaFactor", "type": "range", "method": "getDtiAlphaFactor", "required": "canCurrentOverlayModulate" },
      { "label": "Open in Mango", "action": "OpenInMango", "required": "canOpenInMango" }
    ]
  };

  static PREFERENCES_DATA = {
    "items": [
    ...
    if (menu.items) {
      for (ctr = 0;ctr < menu.items.length;ctr += 1) {
        if (menu.items[ctr].menu) {
          Toolbar.applyContextState(menu.items[ctr].menu)
        } else {
  ...
  buildToolbar() {
    ...
    if (this.container.kioskMode) {
      for (ctr = 0;ctr < Toolbar.MENU_DATA_KIOSK.menus.length;ctr += 1) {
        this.buildMenu(Toolbar.MENU_DATA_KIOSK.menus[ctr], null, this.viewer, null)
      }
    } else {
      if ((this.container.viewer.screenVolumes.length > 0) && this.container.viewer.screenVolumes[0].rgb) {
        Toolbar.MENU_DATA.menus[0] = Toolbar.RGB_FILE_MENU_DATA
      } else {
        if (this.container.noNewFiles) {
          Toolbar.MENU_DATA.menus[0] = Toolbar.RGB_FILE_MENU_DATA
        } else {
          Toolbar.MENU_DATA.menus[0] = Toolbar.FILE_MENU_DATA
        }
        this.buildOpenMenuItems(Toolbar.MENU_DATA)
      }

      for (ctr = 0;ctr < Toolbar.MENU_DATA.menus.length;ctr += 1) {
        this.buildMenu(Toolbar.MENU_DATA.menus[ctr], null, this.viewer, null)
      }
  ...
  buildAtlasMenu() {
    if (Atlas) {
      ...
      items[0] = {
        "label": Atlas.labels.atlas.header.name, "action": "AtlasChanged-" +
          Atlas.labels.atlas.header.name, "type": "radiobutton", "method": "isUsingAtlas"
      }

      if (Atlas.labels.atlas.header.transformedname) {
        items[1] = {
          "label": Atlas.labels.atlas.header.transformedname, "action": "AtlasChanged-" +
            Atlas.labels.atlas.header.transformedname, "type": "radiobutton",
          "method": "isUsingAtlas"
        }
      }
    }
  ...
  buildColorMenuItems() {
    ...
    if (screenParams) {
      for (ctr = 0;ctr < screenParams.length;ctr += 1) {
        ColorTable.addCustomLUT(screenParams[ctr])
      }
    }

    allColorTables = ColorTable.TABLE_ALL
    items = Toolbar.OVERLAY_IMAGE_MENU_DATA.items
  ...
  buildMenu(menuData, topLevelButtonId, dataSource, modifier, context) {
    var menu = null, items
    ...
    if (!menuData.required || ((ObjectUtils.bind(this.container, ObjectUtils.dereferenceIn(this.container, menuData.required)))() === true)) {
      menu = new Menu(this.viewer, menuData, ObjectUtils.bind(this, this.doAction), this.viewer, modifier)
  ...
  buildMenuItems(menu, itemData, topLevelButtonId, dataSource, modifier) {
    ...
    for (ctrItems = 0;ctrItems < itemData.length;ctrItems += 1) {
      if (!itemData[ctrItems].required || ((ObjectUtils.bind(this.container,
        ObjectUtils.dereferenceIn(this.container,
          itemData[ctrItems].required)))(parseInt(modifier)) === true)) {
        if (itemData[ctrItems].type === "spacer") {
          item = new MenuItemSpacer()
        } else if (itemData[ctrItems].type === "radiobutton") {
          item = new MenuItemRadioButton(this.viewer, itemData[ctrItems].label, itemData[ctrItems].action,
            ObjectUtils.bind(this, this.doAction), dataSource, itemData[ctrItems].method, modifier)
        } else if (itemData[ctrItems].type === "checkbox") {
          item = new MenuItemCheckBox(this.viewer, itemData[ctrItems].label, itemData[ctrItems].action,
            ObjectUtils.bind(this, this.doAction), dataSource, itemData[ctrItems].method, modifier)
        } else if (itemData[ctrItems].type === "file") {
          if ((!itemData[ctrItems].hide) && (!itemData[ctrItems].required || ((ObjectUtils.bind(this.container,
            ObjectUtils.dereferenceIn(this.container,
              itemData[ctrItems].required)))(parseInt(modifier)) === true))) {
            item = new MenuItemFileChooser(this.viewer, itemData[ctrItems].label,
              itemData[ctrItems].action, ObjectUtils.bind(this, this.doAction), false, modifier)
          }
          ...
          if ((!itemData[ctrItems].hide) && (!itemData[ctrItems].required || ((ObjectUtils.bind(this.container,
            ObjectUtils.dereferenceIn(this.container,
              itemData[ctrItems].required)))(parseInt(modifier)) === true))) {
            item = new MenuItemFileChooser(this.viewer, itemData[ctrItems].label,
              itemData[ctrItems].action, ObjectUtils.bind(this, this.doAction), true, modifier)
          } else {
            item = null
          }
        } else if (itemData[ctrItems].type === "displayrange") {
          if (this.viewer.screenVolumes[modifier].supportsDynamicColorTable()) {
            item = new MenuItemRange(this.viewer, itemData[ctrItems].label, itemData[ctrItems].action,
              ObjectUtils.bind(this, this.doAction), dataSource, itemData[ctrItems].method, modifier)
          } else {
            item = null
          }
        } else if (itemData[ctrItems].type === "range") {
          if (PlatformUtils.isInputRangeSupported()) {
            item = new MenuItemSlider(this.viewer, itemData[ctrItems].label,
              itemData[ctrItems].action, ObjectUtils.bind(this, this.doAction), dataSource,
              itemData[ctrItems].method, modifier)
          }
        } else {
          item = new MenuItem(this.viewer, itemData[ctrItems].label, itemData[ctrItems].action,
            ObjectUtils.bind(this, this.doAction), dataSource, itemData[ctrItems].method, modifier)
        }
      } else {
        item = null
      ...
      if (item) {
        ...
        if (itemData[ctrItems].items) {
          ...
          item.menu = menu2
          item.callback = ObjectUtils.bind(menu2, menu2.showMenu)
        }
      }
    }
  };
  ...
  doUpdateImageButtons() {
    ...
    if (this.container.showImageButtons) {
      for (ctr = this.viewer.screenVolumes.length - 1;ctr >= 0;ctr -= 1) {
        ...
        if (ctr === 0) {
          if (screenVol.rgb) {
            data.menus[0].items = Toolbar.RGB_IMAGE_MENU_DATA.items
          } else if (screenVol.dti) {
            data.menus[0].items = Toolbar.DTI_IMAGE_MENU_DATA.items
          } else {
            data.menus[0].items = Toolbar.BASE_IMAGE_MENU_DATA.items
          }
        } else {
          if (screenVol.dti) {
            data.menus[0].items = Toolbar.DTI_IMAGE_MENU_DATA.items
          } else {
            data.menus[0].items = Toolbar.OVERLAY_IMAGE_MENU_DATA.items
          }
        }
        ...
        if (surf.staticIcon) {
          var iconCb = function (dataUrl, index) {
            that.surfaceMenus.push((that.buildMenu({
              "label": "SurfaceButton",
              "icons": [dataUrl],
              "items": Toolbar.SURFACE_MENU_DATA.items,
              "imageButton": true,
              "surfaceButton": true
            }, null, that.viewer.surfaces[index], index.toString())))
          }

          ScreenVolume.makeStaticIcon(surf.staticIcon, iconCb, ctr)
        } else {
          ...
          dataUrl = ScreenVolume.makeSolidIcon(solidColor[0], solidColor[1], solidColor[2])
    ...
    if (action) {
      if (action.startsWith("ImageButton")) {
        ...
        for (ctr = 0;ctr < file.length;ctr += 1) {
          ...
          for (ctrI = 0;ctrI < Container.ignorePatterns.length;ctrI += 1) {
            if (Container.ignorePatterns[ctrI].test(file[ctr].name)) {
              ignored = true
            }
          }
        ...
        Container.resetViewer(this.container.containerIndex, {})
      } else if (action === "Preferences") {
        dialog = new Dialog(this.container, "Viewer Preferences", Toolbar.PREFERENCES_DATA,
          this.container.preferences, ObjectUtils.bind(this.container.preferences,
            this.container.preferences.updatePreference),
          ObjectUtils.bind(this,
            function () {
        ...
        dialog = new Dialog(this.container, "Surface Preferences", Toolbar.PREFERENCES_SURFACE_DATA,
          this.container.preferences, ObjectUtils.bind(this.container.preferences,
            this.container.preferences.updatePreference),
          ObjectUtils.bind(this,
            function () {
        ...
        dialog = new Dialog(this.container, "License", Toolbar.LICENSE_DATA,
          Container, null, null, null, true)
        dialog.showDialog()
      } else if (action === "KeyboardRef") {
        dialog = new Dialog(this.container, "Keyboard Reference", Toolbar.KEYBOARD_REF_DATA,
          Container, null, null, null, true)
        dialog.showDialog()
      } else if (action === "MouseRef") {
        dialog = new Dialog(this.container, "Mouse Reference", Toolbar.MOUSE_REF_DATA,
          Container, null, null, null, true)
        dialog.showDialog()
        ...
        if (this.viewer.screenVolumes[imageIndex].volume.numTimepoints > 1) {
          dialog = new Dialog(this.container, "Image Info", Toolbar.SERIES_INFO_DATA,
            this.viewer, null, null, imageIndex.toString())
        } else {
          dialog = new Dialog(this.container, "Image Info", Toolbar.IMAGE_INFO_DATA,
            this.viewer, null, null, imageIndex.toString())
        }
        ...
        dialog = new Dialog(this.container, "Surface Info", Toolbar.SURFACE_INFO_DATA,
          this.viewer, null, null, imageIndex.toString())
        dialog.showDialog()
      } else if (action.startsWith("ShowHeader")) {
        imageIndex = action.substring(action.lastIndexOf("-") + 1)

        dialog = new Dialog(this.container, "Header", Toolbar.HEADER_DATA,
          this.viewer, null, null, imageIndex.toString())
        ...
        if (imageIndex === 0) {
          if (this.container.viewer.volume.urls[0]) {
            PlatformUtils.launchCustomProtocol(this.container, UrlUtils.getAbsoluteUrl(PAPAYA_CUSTOM_PROTOCOL,
  ...
  customProtocolResult(success) {
    if (success === false) { // initiated by a setTimeout, so popup blocker will interfere with window.open
      if ((PlatformUtils.browser === "Chrome") || (PlatformUtils.browser === "Internet Explorer")) {
  ...
  updateImageMenuRange(index, min, max) {
